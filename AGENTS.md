...
使用 Python 3.12 和 PyTorch 生态系统进行深度学习和 LLM 学习与试验。
...
4. 模块化设计
5. 实用主义优于过度工程化
6. 配置管理清晰化 - 使用 argparse + dataclass
...
### PyTorch
- **DO**: `model.train()`/`model.eval()`、`torch.no_grad()`
- **DON'T**: 不手动管理设备
...
- `model.train()` / `model.eval()` 切换模式 → [最佳实践](docs/development/best-practices.md)
- 推理时使用 `torch.no_grad()` 禁用梯度 → [最佳实践](docs/development/best-practices.md)
