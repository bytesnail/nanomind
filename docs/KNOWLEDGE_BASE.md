...
> **快速更新**：`git log <上次commit>..HEAD --oneline` → 分析新 commit → 追加知识点 → 更新元信息
...
- **分波次分析和更新** → 如果待分析commit数量较少，直接分析并更新本文件；如果待分析commit数量很多，应该先将待分析commit按提交内容相关性和数量合理拆分为多个分析波次，且此时只输出多波次分析的详细计划书，不直接开始分析
---
